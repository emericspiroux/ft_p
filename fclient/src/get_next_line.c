...
/*   Updated: 2016/01/14 02:59:19 by larry            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		end(t_buffer *buffer, t_buffer **svg, void *end_line, size_t lg)
{
	if (*svg)
	{
		free((*svg)->data);
...
static int		select_buffer(int fd, t_buffer **buffer, t_buffer *svg_buffer)
{
	int				read_result;
...
static int		read_line(t_buffer *buffer, int fd, char **line, size_t *lgl)
{
	void			*nvline;
	void			*end_line;
